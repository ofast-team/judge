...
class Test:
    ...
    def test_diff_TLE_cpp(self):
        result = compile.compile_and_run("./tests/TLE/hello.cpp", "./tests/TLE/hello", constants.Checker.diff, 5)
        for res in result:
            assert res == constants.Verdict.time_limit
    ...
    def test_diff_TLE_java(self):
        result = compile.compile_and_run("./tests/TLE/hello.java", "./tests/TLE/hello", constants.Checker.diff, 5)
        for res in result:
            assert res == constants.Verdict.time_limit
    ...
    def test_diff_TLE_py(self):
        result = compile.compile_and_run("./tests/TLE/hello.py", "./tests/TLE/hello", constants.Checker.diff, 5)
        for res in result:
            assert res == constants.Verdict.time_limit
   
    # Testing to ensure that the RTE verdict works properly with the c file
    def test_diff_RTE_c(self):
        result = compile.compile_and_run("./tests/RTE/hello.c", "./tests/RTE/hello", constants.Checker.diff, 5)
        for res in result:
    ...
    # Testing to ensure that the RTE verdict works properly with the java file
    def test_diff_RTE_java(self):
        result = compile.compile_and_run("./tests/RTE/hello.java", "./tests/RTE/hello", constants.Checker.diff)
        for res in result:
            assert res == constants.Verdict.runtime_error
   
    # Testing to ensure that the RTE verdict works properly with the python file
